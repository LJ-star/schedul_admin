...
function display_change_config_12(event, clicked_id) {
    if (req_start_flag == 1) {
        $('.panel').css('background', '#F9F9F9');
        flag = 3;
        hidediv.style.display = "block";
        ...
        var rect = schedule_id.getBoundingClientRect();
        // var x = event.clientX;
        // var y = event.clientY;
        var x = rect.left + 20;
        var y = rect.top;
...
function display_add_config(event, clicked_id) {
    // if (edit_flag == 1) {
        ...
        if(event.target.classList[2] == 'chosen')
            display_edit_config_final(event);
...
function display_add_config_final(event) {
    if (edit_flag >= 1) {
        ...
        var y = event.clientY;
        var w = window.innerWidth;
        x = schedule_id[0].style.border = "none";
        var rect = schedule_id[0].getBoundingClientRect();
        // console.log(rect.top, rect.right, rect.bottom, rect.left);
        x = rect.left;
        y = rect.top;
        var temp = x + 300;
        if (w > temp) {
            div_add_shift_position.style.left = Number(x + 12) + 'px';
...
function display_edit_config_final(event) {
    if (edit_flag >= 1) {
        div_edit_shift_position.style.display = "block";
        hidediv.style.display = 'block';
        document.body.style.overflow = 'hidden';
        flag = 1;
        var x = event.clientX;
        ...
        var temp = x + 300;
        if (w > temp) {
            div_edit_shift_position.style.left = x + 'px';
            div_edit_shift_position.style.top = '60px';
        }
        if (w < temp) {
            x = x - 300;
            div_edit_shift_position.style.left = x + 'px';
        ...
        if (w < 580) {
            w = (w - 300) / 2;
            div_edit_shift_position.style.left = w + 'px';
            div_edit_shift_position.style.top = '20px';
        }
        // if (windows zoma < tviton am boxis zomaze) {
        //     div_add_shift_position.style.top = '0px';
        // }
...
function hide_div_edit_shift() {
    ...
    var strOut = extract_id_name.substr(0, index);
    for(let i = 0; i < schedule_id.length; i++){
        schedule_id[i].style.border = "#7B7982 1px solid";
        schedule_id[i].classList.remove(strOut, 'chosen', 'selected', 'editable');
    }
    cancel_template();
}
...
function show_constraints_mobile(event) {
    if(editState){
        var w = window.innerWidth;
        if (w < 800) {
            w = w - 800;
...
function re_edit() {
    ...
    for(let i = 0; i < buttonsToSelect.length; i++){
        ...
        buttonsToSelect[i].classList.remove("selected");
        buttonsToSelect[i].classList.remove("editable");
    }
